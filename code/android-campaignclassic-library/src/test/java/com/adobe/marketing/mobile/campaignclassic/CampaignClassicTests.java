...
public class CampaignClassicTests {
    ...
    public void test_registerDevice_NullToken() {
        try (MockedStatic<MobileCore> mobileCoreMockedStatic = Mockito.mockStatic(MobileCore.class);
            ...
            //verify
            mobileCoreMockedStatic.verify(() -> MobileCore.dispatchEvent(ArgumentMatchers.any()), Mockito.times(0));
            logMockedStatic.verify(() -> Log.error(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any()));
        }
    }
